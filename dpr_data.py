...
import json
from utils import get_passage_file,get_passage_file_dawon
from questiontxt_to_questionjson import read_json_file
# set logger
...
#                 [("제임스 얼 카터 주니어", 0, "제임스 얼 카터 주니어(, 1924년 10월 1일 ~ )는 민주당 출신 미국 39대 대통령 (1977년 ~ 1981년)이다.")]
#             )
#         for item in tqdm(self.raw_json, desc="matching silver passages"): # <-- korquad
#             title = item["title"].replace("_", " ")  # _를 공백문자로 변경 <--korquad
#             #print(item.keys()) #dict_keys(['paragraphs', 'title'])

#             para = item["paragraphs"]
#             cand_ids, refined_title = self._get_cand_ids(title)
# #             print(cand_ids)#None
# #             print(refined_title)#파우스트서곡
...
#                 continue
        ...
        question_list = read_json_file('dataset/question.json')
        assert len(question_list) == len(contexts)
